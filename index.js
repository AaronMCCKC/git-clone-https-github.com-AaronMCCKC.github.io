...
document.getElementById("submitAll").addEventListener("click", (event) => {
  ...
  if (entries[entries.length - 1].length === 0) {
    ...
    errorMessage = ensureRequirementsAreMet(Object.entries(elements).map(element => element[1]));
  }
  ...
  if (errorMessage !== "Error: not all of the required fields were provided.") {
    ...
    try {
      entries.forEach(entry => {
        entry.forEach((field) => {
          entriesString += `${field.name.replace("_", " ")}: ${field.value} <br />`;
        });
      ...
      if (entriesString !== "") document.write(entriesString);
    }
  }
});
